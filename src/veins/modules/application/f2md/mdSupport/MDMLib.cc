...
double MDMLib::calculateDistance(veins::Coord pos1, veins::Coord pos2) {
    return sqrt(pow(pos1.x - pos2.x, 2.0) + pow(pos1.y - pos2.y, 2.0));
    //   + pow(pos1.z - pos2.z, 2.0));
}

double MDMLib::calculateSpeed(veins::Coord Speed) {
    return sqrt(pow(Speed.x, 2.0) + pow(Speed.y, 2.0) + pow(Speed.z, 2.0));
}

double MDMLib::calculateHeadingAngle(veins::Coord heading) {
    double x2 = 1;
    double y2 = 0;
...
double MDMLib::RectRectFactor(veins::Coord c1, veins::Coord c2, double heading1,
        double heading2, veins::Coord size1, veins::Coord size2) {
...
double MDMLib::EllipseEllipseIntersectionFactor(veins::Coord pos1, veins::Coord posConf1,
    ...
    double centersAngle = calculateHeadingAngle(
            veins::Coord(pos2.x - pos1.x, pos2.y - pos1.y));
