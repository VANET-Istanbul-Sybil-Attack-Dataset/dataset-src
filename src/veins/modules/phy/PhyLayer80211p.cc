...
unique_ptr<AnalogueModel> PhyLayer80211p::getAnalogueModelFromName(std::string name, ParameterMap& params)
{
    ...
    else if (name == "DualSlopeModel") {
        return initializeDualSlopeModel(params);
    }
...
unique_ptr<AnalogueModel> PhyLayer80211p::initializeShadowingModel(ParameterMap& params)
{
    bool useTorus = world->useTorus();
    const Coord& playgroundSize = *(world->getPgs());

    ParameterMap::iterator it = params.find("dist0");
    double dist0 = (it != params.end()) ? it->second.doubleValue() : 1.0;

    it = params.find("std_db");
    double std_db = (it != params.end()) ? it->second.doubleValue() : 0.0;

    it = params.find("pathlossExp");
...
unique_ptr<AnalogueModel> PhyLayer80211p::initializeDualSlopeModel(ParameterMap &params) {
    ...
    it = params.find("std1_db");
    double std1_db = (it != params.end()) ? it->second.doubleValue() : 0.0;

    it = params.find("std2_db");
    double std2_db = (it != params.end()) ? it->second.doubleValue() : 0.0;

    it = params.find("pathlossExp1");
    double pathlossExp1 = (it != params.end()) ? it->second.doubleValue() : 2.0;

    it = params.find("pathlossExp2");
    double pathlossExp2 = (it != params.end()) ? it->second.doubleValue() : 6.0;

    it = params.find("dc");
    double dc = (it != params.end()) ? it->second.doubleValue() : 100.0;

    return make_unique<DualSlopeModel>(this, useTorus, playgroundSize, dist0,
            std1_db, std2_db, pathlossExp1, pathlossExp2, dc);
}
