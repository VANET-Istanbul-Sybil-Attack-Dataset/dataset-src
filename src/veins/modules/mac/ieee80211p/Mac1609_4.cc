...
void Mac1609_4::handleSelfMsg(cMessage* msg)
{
    ...
    if (msg == nextChannelSwitch) {
        ...
        BasicSafetyMessage* bsmWithPowerControl = dynamic_cast<BasicSafetyMessage*>(pktToSend);
        if (bsmWithPowerControl && bsmWithPowerControl->getPowerControlTxPower() > 0.1) {
            txPower = FWMath::dBm2mW(bsmWithPowerControl->getPowerControlTxPower());
            txPower_mW = txPower;
            // std::cout << "Mac1609_4::handleSelfMsg powerControl:"  << bsmWithPowerControl->getPowerControl() << std::endl;
        }
        else if (controlInfo) {
